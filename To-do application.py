...
def update_task(tasks, task_index):
    if 1 <= task_index <= len(tasks):
        ...
        print(f"Current Task: {task['description']} - Due Date: {task['due_date']} - Status: {'Complete' if task['complete'] else 'Incomplete'}")
        new_description = input("Enter new task description (press Enter to keep the current description): ")
        new_due_date = input("Enter new due date (press Enter to keep the current due date): ")
